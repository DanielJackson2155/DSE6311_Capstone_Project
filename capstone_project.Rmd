...
reduced_train_df <- cbind(pca_scores, anx_lev_train)
...
For our first model, we are going to build an ordinal regression model. For that we need to first turn our reponse variable into ordinal. 

Firsts lets see the spread of of our variable. 

```{r}
summary(train_df$total_anxiety_level)
...
hist(train_df$total_anxiety_level,
...
train_df$or_total_anxiety_level <- cut(train_df$total_anxiety_level,
                                        breaks = breaks,
                                        labels = labels,
                                        right = TRUE)

#this code shows weather or not it works
head(train_df$or_total_anxiety_level)
#NP
```
```{r}
...
test_df$or_total_anxiety_level <- cut(test_df$total_anxiety_level,
                                        breaks = breaks,
                                        labels = labels,
                                        right = TRUE)

head(test_df$or_total_anxiety_level)
#NP
```
```{r}
#this code is redundant but it ensures that both of the data sets variables are ordinal 

train_df$or_total_anxiety_level <- factor(train_df$or_total_anxiety_level,
                                          levels = 1:9,
                                          ordered = TRUE)


train_df$or_total_anxiety_level <- factor(test_df$or_total_anxiety_level,
                                          levels = 1:9,
                                          ordered = TRUE)
#NP
```
...
ggplot(train_df, aes(x = or_total_anxiety_level)) +
...
table(train_df$or_total_anxiety_level)
summary(train_df$or_total_anxiety_level)
#NP
```
