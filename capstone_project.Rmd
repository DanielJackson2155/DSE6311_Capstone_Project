...
# We will use the average of total_anxiety_level as threshold to create binary classifier which will be our response variable for our binary classification models that we build
avg_tot_anx_lev = mean(cov_df$total_anxiety_level)
# Mean is 251.17

# Now, let us create change_anxiety response variable with two levels: "yes" if the value is above the average and "no" if value is below the average
cov_df = cov_df %>%
  mutate(change_anxiety = ifelse(total_anxiety_level > avg_tot_anx_lev, "yes", "no"))

# Now, we can remove the total_anxiety_level variable
cov_df = cov_df[, -which(names(cov_df) =="total_anxiety_level")]
