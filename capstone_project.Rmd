...
Let us split our data set into test and training data. 70% of our data will be the training data and 30% will be our test data. The trained data will be used to train our models while the test data will be used to test the accuracy of the trained model's predictions.
```{r 18}
set.seed(123) # set.seed(123) for reproducibility
train <-sample(nrow(cov_df), 0.7 * nrow(cov_df))
train_df = cov_df[train, ]
test_df = cov_df[-train, ]
...
We are left with our response variable: total_anxiety_level.
We are also left with 25 predictors.
