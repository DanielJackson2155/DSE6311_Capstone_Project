...
```{r}
#unscaling the "test_feat" so it so predictors can be plotted against response.


#attaching test predictions to testing set
test_feat <- cbind(test_feat, predictions)

#function created to plot predictors against response variables to see patterns within students.
plot_list <- list()

for (predictor in colnames(test_feat)[colnames(test_feat) != "predictions"]) {
  p <- ggplot(test_feat, aes_string(x = predictor, y = "predictions")) +
       geom_point() +
